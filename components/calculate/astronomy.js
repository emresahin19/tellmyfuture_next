...
export function cnvCalendar( JD ){
	JD += 0.5;
	var Z = Math.floor( JD );
	var F = JD - Z;
...
export function calJD(ye, mo, da, ho, mi){
	var y0 = (mo > 2) ? ye : (ye -  1);
...
export function calJDz(year, month, day){ 
	var yt = year;
	var mt = month;
	var dt = day;
...
export function calTimeCoefficient( JD ){
	var d = JD - 2451545.0;
	var T = d / 36525.0;
...
export function convertOrbitalElement( a, l, h, k, p, q ) {
	var L = l;
...
export function orbitWork(L, opi, omg, i, e, a){
	var M = mod360(L - opi);
...
export function convertGeocentric( earthCoor, planetCoor ){
...
export function convertEquatorial(lon, lat, obl){
...
export function coordinateConvertFromJ2000( arg ){
	var zeta, zz, theta;
...
export function calLST(JD, ho, mi, lo){
...
export function calNutation( JD ){
	var T = (JD - 2451545.0) / 36525.0;
...
export function calDayOfWeek(year, month, day){
	var JD = calJDz(year, month, day);
	var you = (JD + 1) % 7;
	return you;
}

export function chkLeap( year ){
	var chk = 0;
...
export function advanceDate( date, step ){
	return encodeDate(cnvCalendar(calJDz(decodeDate(date)) + step));
}

export function calDist(sy, sm, sd, ey, em, ed){
...
export function encodeDate(ye, mo, da){
	return ye * 10000 + mo * 100 + da;
}

export function encodeTime(ho, mi){
	return ho * 100 + mi;
}
